"""
Fast ANACOFI + AFFO scrape.

Lightweight job that scrapes only ANACOFI (CSV export) and AFFO (HTML),
then merges ADDITIVELY into existing member data. Never drops existing
members. Runs in ~5 minutes — well within GitHub Actions timeout.
"""
import json
import logging
import os
...
def main():
    ...
    try:
        anacofi = scrape_anacofi()
        elapsed = (datetime.now(timezone.utc) - t0).total_seconds()
        logger.info(f"<<< ANACOFI done in {elapsed:.0f}s ({len(anacofi)} members)")
        scraped.extend(anacofi)
        scrape_status["anacofi"] = {"status": "success", "count": len(anacofi),
                                    "timestamp": now_iso, "duration_s": round(elapsed, 1)}
    except Exception as e:
        elapsed = (datetime.now(timezone.utc) - t0).total_seconds()
        logger.error(f"<<< ANACOFI FAILED after {elapsed:.0f}s: {e}")
        scrape_status["anacofi"] = {"status": "error", "error": str(e),
                                    "timestamp": now_iso, "duration_s": round(elapsed, 1)}
    ...
    try:
        affo = scrape_affo()
        elapsed = (datetime.now(timezone.utc) - t0).total_seconds()
        logger.info(f"<<< AFFO done in {elapsed:.0f}s ({len(affo)} members)")
        scraped.extend(affo)
        scrape_status["affo"] = {"status": "success", "count": len(affo),
                                 "timestamp": now_iso, "duration_s": round(elapsed, 1)}
    except Exception as e:
        elapsed = (datetime.now(timezone.utc) - t0).total_seconds()
        logger.error(f"<<< AFFO FAILED after {elapsed:.0f}s: {e}")
        scrape_status["affo"] = {"status": "error", "error": str(e),
                                 "timestamp": now_iso, "duration_s": round(elapsed, 1)}

    if not scraped:
        logger.error("No members scraped from any source — aborting")
        return

    # Merge: existing members + freshly scraped ANACOFI/AFFO
    logger.info(">>> Merging...")
    all_members = merge_all_sources(existing_members, scraped)
